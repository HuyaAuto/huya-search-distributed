 ...
 * @date 2018年03月26日
 */
class Taf_log_dataConvert extends AbstractDataConvert<byte[], byte[], SearchDataRow> {
    ...
    protected SearchDataRow realConvert(int id, long offset, byte[] line, byte[] key) throws IgnorableConvertException {
        ...
        try {
            LogSvrRecord logSvrRecord = LogSvrRecord.parseRawRecord(key, line);

            app = logSvrRecord.getApp()
            module = logSvrRecord.getModule()
            type = logSvrRecord.getType()
            ...
            DynamicAnalyzer dynamicAnalyzer = IndexSettings.getDynamicAnalyzer(logSvrRecord.getApp(), logSvrRecord.getModule(), logSvrRecord.getType());
            if (dynamicAnalyzer != null && StringUtils.isNotEmpty(logSvrRecord.getData())
                    && logSvrRecord.getData().length() > 5
                    && StringUtils.isNotEmpty(logSvrRecord.getData().trim()) && logSvrRecord.getData().split(dynamicAnalyzer.getChars()).length < 3) {
                logger.info(logSvrRecord.getApp() + "-" + logSvrRecord.getModule() + "-" + logSvrRecord.getType() + ":" + logSvrRecord.getData())
            }

            // 写入文件内容
            items.add(SearchDataItem.newInstance(CONTENT, logSvrRecord.getData(), dynamicAnalyzer));
            ...
            throw new IgnorableConvertException(app + "-" + module + "-" + type + "unknown error", e);
        }
    }
}
...
public class DisDSLSortAggrQueryContext extends DisDSLDefaultAggrQueryContext implements DisQueryContext {
    ...
    public NodeMerger getNodeMerger() {
        ...
        AggrFunSet aggrFunSet = getSelectItems().getNodeAggrFunSet();
        Sort sort = getSortedItems().sortInstance();
        if (groupByItems == null) {
            SortMerger sortMerger = new SortMerger(sort);
            return AggrNodeMerger.newInstance(new AggrSortMerger(aggrFunSet, sortMerger));
        }
        else {
            return AggrNodeMerger.newInstance(new AggrSortGroupMerger(aggrFunSet, sort, groupByItems.groupFields(), getMetaDefine()));
        }
    }

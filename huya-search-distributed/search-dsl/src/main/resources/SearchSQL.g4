/**
 * Created by zhangyiqun1@yy.com on 2017/9/07.
 */
grammar SearchSQL;
...
cursor_select_stmt
 : K_CURSOR ':' simple_select_stmt
 ;

cursor_stmt
 : K_CURSOR ':' STRING_LITERAL
 ;
...
simple_select_stmt
 : K_SELECT  result_column ( ',' result_column )*
   ...
   ( K_WHERE where_expr )?
   ( K_GROUP K_BY group_column ( ',' group_column )* )?
   ( K_ORDER K_BY order_column ( ',' order_column )* )?
   ( K_LIMIT limit)?
 ;

result_column
 : '*'
 | function_expr ( K_AS? column_alias )?
...
function_expr
 : function_name '(' ('*' | column_name (',' column_name)* ) ')'
 ;

where_expr
 : '(' where_expr ')'
 | where_expr K_AND where_expr
 | where_expr K_OR where_expr
 | column_name current_operator literal_value
 ;

 ...
 */
public class AndWhereStatement implements WhereStatement {

    private List<WhereStatement> statements;

    public static WhereStatement create(List<WhereStatement> statements) {
        return new AndWhereStatement(statements);
    }

    private AndWhereStatement(List<WhereStatement> statements) {
    ...
    public PartitionRange getPartitionRange() {
        ...
        if (statements.size() == 1) {
            ...
            for (int i=1; i<statements.size(); i++) {
                range = PartitionRange.intersection(range, statements.get(i).getPartitionRange());
            }
            return range;
        }
    }
    ...
    public BooleanQuery getQuery(IntactMetaDefine intactMetaDefine, String logSvrApp, String logSvrModule, String logSvrType) throws BuildLuceneQueryException {
        ...
        if (statements.size() == 1) {
            ...
            for (WhereStatement statement : statements) {
                bqb.add(statement.getQuery(intactMetaDefine, logSvrApp, logSvrModule, logSvrType), BooleanClause.Occur.FILTER);
            }
            return bqb.build();
        }
    }

    public static class AndWhereStatementSerializer extends Serializer<AndWhereStatement> {

        @Override
        public void write(Kryo kryo, Output output, AndWhereStatement object) {
        ...
        @Override
        public AndWhereStatement read(Kryo kryo, Input input, Class<AndWhereStatement> type) {
